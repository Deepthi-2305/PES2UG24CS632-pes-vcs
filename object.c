...
if (fread(buffer, 1, file_size, fp) != (size_t)file_size) {
    ...
    char *nullpos = memchr(buffer, '\0', file_size);
    ...
    *space = '\0';
    ...
    if (strcmp(buffer, "blob") == 0) *type_out = OBJ_BLOB;
    else if (strcmp(buffer, "tree") == 0) *type_out = OBJ_TREE;
    else *type_out = OBJ_COMMIT;

    size_t len = atoi(space + 1);
    *len_out = len;

    *data_out = malloc(len);
    memcpy(*data_out, nullpos + 1, len);
